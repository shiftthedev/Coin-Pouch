...
public abstract class AbstractContainerScreenMixin<T extends AbstractContainerMenu> extends Screen implements MenuAccess<T> {
    @Shadow public abstract @NotNull T getMenu();
    ...
    private void coinPouchCount(ItemRenderer itemRenderer, Font font, ItemStack itemStack, int x, int y, String count) {
        if (this.getMenu() instanceof CoinPouchContainer && itemStack.getCount() > 1 && !Screen.hasShiftDown()) {
            itemRenderer.renderGuiItemDecorations(font, itemStack, x, y, VaultCoinPouch.formatCount(itemStack.getCount()));
        } else {
