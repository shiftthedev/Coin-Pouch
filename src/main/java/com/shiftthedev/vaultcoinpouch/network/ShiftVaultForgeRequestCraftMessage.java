...
public class ShiftVaultForgeRequestCraftMessage
{
    ...
    public static void handle(ShiftVaultForgeRequestCraftMessage message, Supplier<NetworkEvent.Context> contextSupplier)
    {
        ...
        context.enqueueWork(() -> enqueueWork(context, message));
    ...
    private static void enqueueWork(NetworkEvent.Context context, ShiftVaultForgeRequestCraftMessage message)
    {
        ...
        if (requester != null)
        {
            ...
            if (patt2079$temp instanceof ForgeRecipeContainer)
            {
                ...
                if (ShiftInventoryUtils.consumeInputs(recipe.getInputs(), playerInventory, tileInventory, true) &&
                        ShiftInventoryUtils.consumeInputs(recipe.getInputs(), playerInventory, tileInventory, false, consumed))
                {
